...
def upsert_rows(conn, table: str, columns: List[str], rows: List[List[Any]]):
    ...
    # --- safety casting: Bitrix иногда присылает boolean для numeric UF_*.
    # Если в Postgres колонка числовая, а значение bool -> приводим к 0/1, чтобы не было DatatypeMismatch.
    global _PG_COLTYPES_CACHE
    try:
        _PG_COLTYPES_CACHE
    except NameError:
        _PG_COLTYPES_CACHE = {}

    def _get_pg_coltypes(_conn, _table: str) -> Dict[str, str]:
        key = str(_table).lower()
        if key in _PG_COLTYPES_CACHE:
            return _PG_COLTYPES_CACHE[key]

        sch, tbl = ("public", _table)
        if "." in _table:
            sch, tbl = _table.split(".", 1)

        with _conn.cursor() as _cur:
            _cur.execute(
                """
                SELECT lower(a.attname) AS column_name,
                       lower(pg_catalog.format_type(a.atttypid, a.atttypmod)) AS pg_type
            ...
            m = {r[0]: r[1] for r in _cur.fetchall()}

        _PG_COLTYPES_CACHE[key] = m
        return m

    try:
        coltypes = _get_pg_coltypes(conn, table)
        numeric_markers = ("double", "real", "numeric", "decimal", "int", "float")
        idx_map = {c: i for i, c in enumerate(col_order)}
        for r in rows:
            for c in col_order:
                i = idx_map[c]
                v = r[i]
                if isinstance(v, bool):
                    dt = (coltypes.get(str(c).lower()) or "")
                    if any(x in dt for x in numeric_markers):
                        r[i] = 1 if v else 0
    except Exception:
        # никогда не блокируем синк из-за этой страховки
        pass
...
# NOTE: normalize_value is defined выше (с обработкой numeric<-bool и прочими нормализациями).
