...
def upsert_rows(conn, table: str, columns: List[str], rows: List[List[Any]]):
    ...
    # --- safety casting (server-side types): if a Postgres numeric column receives bool -> cast to 0/1.
    # Bitrix sometimes returns false/true for numeric UF_* fields; psycopg then sends boolean and Postgres rejects it.
    ...
    try:
        ...
        for i, c in enumerate(col_order):
            ...
            if any(x in dt for x in ("double", "real", "numeric", "decimal", "int", "float")):
                numeric_cols.add(i)

        if numeric_cols:
            for r in rows:
                for j in numeric_cols:
                    if j < len(r) and isinstance(r[j], bool):
                        r[j] = 1 if r[j] else 0
    except Exception:
        # never block sync on casting helper
        pass
    with conn.cursor() as cur:
