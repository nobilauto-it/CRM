...
def _money_to_num(v: Any) -> float:
    """Extract first numeric value from strings like '100 MDL/MLK/MLC/MLT' or return float(v)."""
    if v is None:
        return 0.0
    ...
    if isinstance(v, (list, tuple)) and v:
        return _money_to_num(v[0])
    if isinstance(v, dict):
        for k in ("VALUE", "value", "AMOUNT", "amount"):
            if k in v:
                return _money_to_num(v.get(k))
        return 0.0
    s = str(v).strip()
    if not s:
        return 0.0
...
def _fmt_money_num(n: float, suffix: str = "MDL") -> str:
    """Format number for reports (avoid 100.0)."""
    if abs(n - round(n)) < 1e-9:
        return f"{int(round(n))} {suffix}"
    nn = round(n, 2)
    return f"{nn:g} {suffix}"
...
def pg_list_stock_raw(
    ...
    sql = f"""
        WITH t AS (
            SELECT 
                id, 
                raw, 
                COALESCE(raw->'fields', raw) AS src,
        ...
        WHERE (
            (jsonb_typeof(t.src->'{branch_field_escaped}') = 'string' AND (t.src->>'{branch_field_escaped}') = %s)
            OR
            (jsonb_typeof(t.src->'{branch_field_escaped}') = 'number' AND (t.src->>'{branch_field_escaped}') = %s)
            OR
            (jsonb_typeof(t.src->'{branch_field_escaped}') = 'object' AND (
                COALESCE((t.src->'{branch_field_escaped}')->>'id', (t.src->'{branch_field_escaped}')->>'ID', '') = %s
                OR COALESCE((t.src->'{branch_field_escaped}')->>'value', (t.src->'{branch_field_escaped}')->>'VALUE', '') = %s
                OR COALESCE((t.src->'{branch_field_escaped}')->>'name', (t.src->'{branch_field_escaped}')->>'NAME', '') = %s
            ))
            OR
            (jsonb_typeof(t.src->'{branch_field_escaped}') = 'array' AND EXISTS (
                SELECT 1
                FROM jsonb_array_elements(t.src->'{branch_field_escaped}') e
                WHERE
    ...
    params: List[Any] = [
        # Строка 729: (t.src->>field) = %s - 1 параметр
        branch_value_str,
        # Строка 731: (t.src->>field) = %s - 1 параметр
        branch_value_str,
        # Строка 734: COALESCE(...) = %s - 1 параметр
        branch_value_str,
        # Строка 735: COALESCE(...) = %s - 1 параметр
        branch_value_str,
        # Строка 736: COALESCE(...) = %s - 1 параметр
        branch_value_str,
        # Строка 743: trim(...) = %s - 1 параметр
        branch_value_str,
        # Строка 746: COALESCE(e->>'id', ...) = %s - 1 параметр
        branch_value_str,
        # Строка 747: COALESCE(e->>'value', ...) = %s - 1 параметр
        branch_value_str,
        # Строка 748: COALESCE(e->>'name', ...) = %s - 1 параметр
        branch_value_str,
    ]
    # Итого в WHERE: 9 параметров (только branch_value_str)
...
def _build_deals_auto_date_rows(deals: List[Dict[str, Any]], enum_map_sursa: Optional[Dict[str, str]] = None) -> List[List[Any]]:
    ...
    for idx, d in enumerate(deals):
        ...
        total = ""
        opportunity_raw = _row_get_any(d, raw, "OPPORTUNITY")
        opportunity_num = _money_to_num(opportunity_raw)

        service_add_raw = _row_get_any(d, raw, DEALS_F_SERVICE_ADDITIONAL.upper())
        if service_add_raw is None:
            service_add_raw = _row_get_any(d, raw, DEALS_F_SERVICE_ADDITIONAL)
        service_add_num = _money_to_num(service_add_raw)

        # Total = Opportunity + Service additional (суммируем по числу без учёта кода валюты)
        total_sum_num = opportunity_num + service_add_num
        if total_sum_num < 0:
            total_sum_num = 0.0

        # форматируем service additional для отображения
        service_additional = ""
        if service_add_num > 0:
            service_additional = _fmt_money_num(service_add_num, "MDL")

        # pret/zi
        pret_zi_num = total_sum_num / float(zile) if zile > 0 else 0.0
        pret_zi = _fmt_money_num(pret_zi_num, "MDL")

        # pret/zi (EUR) по курсу из env (по умолчанию 20)
        eur_rate = _money_to_num(os.getenv("EUR_RATE", "20")) or 20.0
        pret_zi_euro = _fmt_money_num((pret_zi_num / eur_rate) if eur_rate else 0.0, "EUR")

        total = _fmt_money_num(total_sum_num, "MDL")
out.append(
            [
                deal_no,
                assigned_name,
...
def generate_pdf_stock_auto_split(
    ...
    # Используем weasyprint если доступен, иначе ReportLab
...
def _generate_pdf_stock_auto_split_reportlab(
    ...
    if deals_auto_date is not None:
        ...
        # Подсчитываем сумму в колонке "Total suma" (индекс 11, после добавления колонки евро)
        total_sum = 0
    service_sum = 0
        if deal_rows:
            for row in deal_rows:
                if len(row) > 11:
                    total_str = str(row[11]) if row[11] else ""
        ...
        if deal_rows and total_sum > 0:
            total_row = [_p("", cell) for _ in range(len(deals_header) - 1)]  # Пустые ячейки (11 штук)
            total_row.append(_p(f"{total_sum} MDL", cell, bold=True))  # Итоговая сумма в последней колонке (индекс 11)
...
def _generate_pdf_stock_auto_split_weasyprint(
    ...
    def generate_donut_chart_svg(
        ...
        # Цвета для сегментов (зависят от среднего количества дней)
        colors = {
            'service': get_red_color(avg_days_service, max_days),
            'chirie': get_green_color(avg_days_chirie, max_days),
            'disponibile': get_red_color(avg_days_disponibile, max_days),
            'alte': get_red_color(avg_days_alte, max_days)
        }
    ...
    <head>
        ...
        <style>
            ...
            .stock-auto-page {{
                page-break-after: always;  /* Принудительный разрыв после STOCK AUTO таблиц */
            }}
            ...
            .left-column {{
                flex: 0 0 350px;  /* Увеличена ширина для диаграммы и легенды */
                display: flex;
            ...
            .grid-container {{
                ...
                grid-template-rows: auto auto;  /* Автоматическая высота вместо 1fr */
                gap: 1mm;  /* Минимальный отступ между таблицами */
            }}
            .table-section {{
                display: flex;
                flex-direction: column;
            ...
            .table-section h3 {{
                font-size: 12pt;  /* Уменьшен (было 20pt) */
                margin: 0 0 1mm 0;  /* Уменьшен отступ снизу */
                font-weight: bold;
            }}
            table {{
                width: 100%;
            ...
            .table-section table td:nth-child(7) {{
                width: 6mm;  /* Zile - маленькие числа */
            }}
    ...
    <body>
        <div class="stock-auto-page">
            <h1>STOCK AUTO — {html.escape(branch_name)}</h1>
    ...
    service_sum = 0
        ...
        total_row_index = -1
        if deal_rows_with_total and total_sum > 0:
            total_row = [""] * len(deals_header)
    total_row[-2] = f"{service_sum} MDL"
    total_row[-1] = f"{total_sum} MDL"
deal_rows_with_total.append(total_row)
    ...
    <head>
        ...
        <style>
            ...
            .table-section h3 {{
                font-size: 12pt;  /* Увеличен шрифт заголовков секций */
                margin: 0 0 1mm 0;  /* Уменьшен отступ снизу */
                font-weight: bold;
            }}
            table {{
                width: 100%;
                border-collapse: collapse;
                font-size: 8pt;  /* Увеличен шрифт данных */
                table-layout: fixed;
            }}
            .deals-table {{
                font-size: 12pt;  /* Уменьшен (было 24pt) */
            ...
            .deals-page {{
                page-break-before: auto;  /* Убрали принудительный разрыв страницы */
                margin-top: 3mm;  /* Небольшой отступ сверху */
                margin-bottom: 3mm;  /* Небольшой отступ снизу */
            }}
            .deals-page:first-of-type {{
                page-break-before: always;  /* Принудительный разрыв для первой таблицы deals */
                margin-top: 0;
            }}
        </style>
    </head>
    ...
    try:
        ...
        # Если weasyprint не работает, используем ReportLab fallback
        print(f"ERROR: weasyprint failed: {e}", file=sys.stderr, flush=True)
        import traceback
        print(f"ERROR: weasyprint traceback:\n{traceback.format_exc()}", file=sys.stderr, flush=True)
        print("Falling back to ReportLab...", file=sys.stderr, flush=True)
        try:
            return _generate_pdf_stock_auto_split_reportlab(
                raw_items, branch_name, branch_id, branch_field, branch_id_name_map,
                enum_map_brand, enum_map_model, deals_auto_date, enum_map_sursa,
            ...
            print(f"ERROR: ReportLab fallback also failed: {reportlab_error}", file=sys.stderr, flush=True)
            import traceback
            print(f"ERROR: ReportLab traceback:\n{traceback.format_exc()}", file=sys.stderr, flush=True)
            raise
...
def send_stock_auto_reports_filtered(
    ...
    try:
        ...
        # Подсчитываем Total venit astăzi (просто суммируем все суммы)
        total_venit_astazi = 0.0
        ...
        caption += f"<b>Total venit astăzi - {int(round(total_venit_astazi))} MDL</b>\n"
...
def send_stock_auto_reports(
    ...
    try:
        ...
        for idx, (display_name, filter_value) in enumerate(branches_ordered, start=1):
            ...
            # ВАЖНО: Для Ungheni гарантируем обработку ВСЕГДА
            ...
            try:
                ...
                # Суммируем opportunity из deals_auto_date
                if deals_for_pdf:
                    try:
                        for deal in deals_for_pdf:
                            try:
                                opp = deal.get("opportunity") if deal else None
                                if opp:
                                    val = float(opp) if not isinstance(opp, str) else float(str(opp).replace(',', '.').replace(' ', ''))
                                    if val > 0:
                                        total_venit += val
                            except:
                                pass
                ...
                caption += f"<b>Total venit astăzi - {int(round(total_venit))} MDL</b>\n"
                caption += f"<b>Auto - {total_auto}</b>\n\n"
                caption += f"Mașini date - <b>{deals_auto_date_count}</b>\n"
                caption += f"Mașini primite - <b>{deals_primite_count}</b>\n"
                caption += f"Mașini prelungite - <b>{deals_prelungire_count}</b>\n\n"
